...
diag.add_field('atmosphere', 'condensation_rain', time_avg=True) #large-scale

diag.add_field('mixed_layer', 't_surf', time_avg=True)
diag.add_field('mixed_layer', 'flux_t', time_avg=True)   #SH
diag.add_field('mixed_layer', 'flux_lhe', time_avg=True) #LH - this is evap if / L_v
diag.add_field('mixed_layer', 'albedo', time_avg=True) 

# lw rad fluxes
diag.add_field('two_stream', 'surf_lw_down',     time_avg=True) #lw_surf_down
diag.add_field('two_stream', 'b_surf',           time_avg=True) #lw_surf_up
diag.add_field('two_stream', 'olr',              time_avg=True) #lw_toa_up (down is zero)
# sw rad fluxes
diag.add_field('two_stream', 'net_surf_sw_down', time_avg=True) #sw_surf_net. For sw_surf_up correct using albedo
diag.add_field('two_stream', 'toa_sw_in',        time_avg=True) #sw_toa_down
#as there is no sw absorption sw_surf_up = sw_toa_up
...
exp.namelist = namelist = Namelist({
    ...
    'two_stream_gray_rad_nml': {
        ...
        'solday':90,                         # seasonal cycle needed for diurnal cycle
        'atm_abs': 0.2,                      # default: 0.0        
        'solar_constant':1365.0,             # default is 1360.
    },
...
if __name__=="__main__":
    ...
    for i in range(2,121): #approx a 2 year spin up and 
